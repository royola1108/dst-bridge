...
Layer 3: Claude (策略层, 贵, 慢, 几分钟一次)
  ...
  dst wait             ← 阻塞等待, goal 完成/失败/被 reflex 中断时立即返回
  ...
  典型循环:
    dst goal "gather logs 20"
    dst wait
      → "interrupted: attacked by spider, reflex fled to campfire"
    dst situation
      → HP:80, night coming, near campfire
    dst goal "survive-night"
    dst wait
      → "done: dawn arrived"
    dst goal "gather logs 20"    # 继续昨晚没完成的
    dst wait
      → "done: 20 logs collected"
...
Layer 2: Bridge Server (agent runtime, 本地, 毫秒级)
  ...
  ├── Wait Manager      ← dst wait 长轮询: 有结果立即返回, 超时返回心跳
...
│  │   POST /api/wait        → 长轮询阻塞, goal 终态时立即返回      │
...
dst goal "build science_machine"
dst wait [timeout]         # 阻塞等待当前 goal 完成/失败/被中断, 立即返回结果
...
| 条件 | 返回示例 |
|---|---|
| goal 完成 | `done: gathered 20 logs (3m 12s)` |
| goal 失败 | `failed: no trees nearby, explored 50m radius` |
| reflex 中断了 goal | `interrupted: attacked by hound, reflex fled to campfire` |
| 角色死亡 | `emergency: player died, cause: hound (Day 3)` |
| 超时（默认 120s） | `timeout: goal still running (12/20 logs), use 'dst wait' again` |
| 手动 interrupt | `cancelled by user` |
...
goal: gather logs 20
status: interrupted
reason: attacked by hound
reflex: fled to campfire (auto)
elapsed: 45s
progress: 12/20 logs collected

current situation:
  HP:80/150 Hgr:90/150
  Day3 Autumn night
  Near campfire, safe
...
**Input:**
```json
{ "playerUserId": "KU_xxx", "timeout": 120 }
```
**行为**: 长轮询。Bridge 阻塞直到以下任一条件满足，然后立即返回：
- 当前 goal 完成 (`done`)
- 当前 goal 失败 (`failed`)
- reflex 中断了 goal (`interrupted`)
- 角色死亡 (`emergency`)
- 超时 (`timeout`)，返回当前进度让 Claude 再次 `dst wait`
...
status: done
elapsed: 2m 15s
result: 20 logs collected (was 5, now 25)

suggested next: dst goal "build campfire" or dst situation
```

**Output (无 goal 时):**
```
no active goal — use 'dst goal' to start one
```
...
elapsed: 120s (timeout)
progress: 15/20 logs collected

use 'dst wait' to continue waiting
```

**实现**: Bridge 内部轮询 macro executor 状态（每 200ms 检查一次），有终态立即响应。HTTP 保持连接打开直到返回。如果 Claude/CLI 断开连接，Bridge 清理 wait session 但不影响 goal 执行。
...
dst wait [timeout]         # Block until current goal finishes/gets interrupted
